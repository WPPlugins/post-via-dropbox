	...
	public function adminpage() {
		...
		<script>
			jQuery(document).ready(function() {
				jQuery('.pvd_linkAccount').click(function() {
...
<li><strong>[post_type]</strong> Post Type (e.g. 'post', 'page', etc) <strong>[/post_type]</strong></li>
	...
	private function insert($posts) {
		...
		foreach ($posts as $post) {
			...
			if ($options['simplified']) {
				extract( $this->parse_simplified($res) );
			} else {
			...
			$post_array = array(	'ID'				=>	( $id ? intval($id) : (preg_match('#^(\d+)-#', pathinfo($post, PATHINFO_BASENAME), $matched) ? $matched[1] : null ) ),
									...
									'post_excerpt' 		=>	( $excerpt ? $excerpt : null ),
	...
	private function parse_simplified ($input) {
		$title = trim(pathinfo($input['name'], PATHINFO_FILENAME));
		$title = preg_replace('#^[\d-]+#', '', $title);
		$title = urldecode($title);
		$content = trim($input['data']);
		return array(
				'title'=>$title,
				'content'=>$content
					);
